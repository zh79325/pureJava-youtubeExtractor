...
public class SSLocal {
    ...
    public void start(String configPath,SSLocalHander hander) throws Exception {
        final Config config = ConfigLoader.load(configPath);
        start(config,hander);
    }

    public void start(Config config,SSLocalHander hander) throws Exception {
        ...
        for (Map.Entry<Integer, String> portPassword : config.getPortPassword().entrySet()) {
            startSingle(config.getLocalAddress(), config.getLocalPort(),
                    ...
                    config.getMethod(),
                    config.getObfs(),
                    config.getObfsParam(),hander);
        }
    }

    private void startSingle(String socks5Server, Integer socks5Port, String server, Integer port, String password, String method, String obfs, String obfsparam,SSLocalHander hander) throws Exception {
...
//                                .addLast(new LoggingHandler(LogLevel.INFO))
    ...
    public static void main(String[] args) throws Exception {
        try {
            getInstance().start("conf/config-example-client.json",null);
        } catch (Exception e) {
            e.printStackTrace();
            getInstance().stop();
            System.exit(-1);
