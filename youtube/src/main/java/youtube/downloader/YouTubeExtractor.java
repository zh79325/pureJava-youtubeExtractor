...
public abstract class YouTubeExtractor extends AsyncTask<String, Void, Map<Integer, YtFile>> {
    ...
    public YouTubeExtractor(Path cacheFolder) {
        cacheDirPath = cacheFolder.toAbsolutePath().toString();
    }
    ...
    @Override
    protected Map<Integer, YtFile> doInBackground(String... params) {
        videoID = null;
    ...
    private Map<Integer, YtFile> getStreamUrls() throws IOException, InterruptedException {
        ...
        urlConnection = (HttpURLConnection) getUrl.openConnection(getProxy());
        ...
        if (encSignatures != null) {
            ...
            if (signature == null) {
                ...
                List<Integer> keys = new ArrayList<>(encSignatures.keySet());
    ...
    private void parseDashManifest(String dashMpdUrl, Map<Integer, YtFile> ytFiles) throws IOException {
        ...
        HttpURLConnection urlConnection = (HttpURLConnection) getUrl.openConnection(getProxy());
        urlConnection.setRequestProperty("User-Agent", USER_AGENT);
        try {
            reader = new BufferedReader(new InputStreamReader(urlConnection.getInputStream()));
            reader.readLine();
    ...
    private void decipherViaWebView(final Map<Integer, String> encSignatures) {
        ...
        try {
            ...
            nashorn.eval(stb.toString());
            decipheredSignature = writer.toString();
        } catch (Exception e) {
